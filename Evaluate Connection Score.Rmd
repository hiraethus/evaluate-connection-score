...
As we can see from the plot, Connection Score grows exponentially as m converges on N. More importantly, as m converges on N, the connection score tends towards 1.0. This means that connection 
...
p.vals <- apply(result, 1, function(row) {
  ...
  random.connection.scores <- lapply(random.signatures, function(rand.prof) {
...
This would imply that connection scores, if viewed in isolation might be misleading where they are 
extremely accurate.
